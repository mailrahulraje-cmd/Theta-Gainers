...
class TelegramNotifierTextOnly:
    ...
    def start(self):
        ...
        try:
            startup_msg = " <b>NOTIFIER OK</b>\n System notifier initialized and ready"
    ...
    def _snapshot_loop(self):
        ...
        while not self._stop_event.is_set():
            try:
                now = time.time()
                # Atomic snapshot scheduling: only send if interval elapsed
                with self._lock:
                ...
                if should_send:
                    ...
                    if snapshot_text:
                        ...
                        # Update last_snapshot only after attempting send to avoid duplicates
                        if sent:
                            with self._lock:
